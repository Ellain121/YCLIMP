...
File_Ptr FsFile::InitFile(Client& client)
{
    ...
    return std::shared_ptr<File>(
        new FsFile{client, "Music", home_dir + "/Music", "NULL", File::Type::Directory});
}
 ...
 * @param onlyMusicExtensions if true regex_match to
 * std::make_shared<File>(this) (false by default)
 * ".*\\.(?:mp3|mp2|mp1|ogg|wav|aiff|flac)"
 * @param recursively default:false if true traverse through all folders
 * recursively
 */
    ...
    if (recursively)
    {
        ...
        for (auto iter{begin}; iter != end; ++iter)
        {
            ...
            if (onlyMusicExtensions == false || IsFileHasMusicExtension(name))
            {
                ...
                File::Type  type = fs::is_directory(entry) ? File::Type::Directory
                                                           : File::Type::Track;
    ...
    File* file, bool noDirs, bool onlyMusicExtensions, bool recursively)
{
    ...
    //                                                   : File::Type::Track;
    //         files.push_back(
    //             std::shared_ptr<File>{new FsFile{client, name, path,
    //             location_id, type}});
    ...
    File* file, bool noDirs, bool onlyMusicExtensions, bool recursively)
{
    ...
    // Запрос может быть как в главной дире /App, так и в альбомах
    // /App/playlist_name Если к главной дире, то просто берем список плейлистов
    // из бдшки и возвращаем Если к плейлисту, то берем список треков в плейлисте
    //		Нюанс в том, что треки в плейлисте будут как из local так и из
    // yandex
}
