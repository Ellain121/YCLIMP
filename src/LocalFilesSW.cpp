...
LocalFilesSW::LocalFilesSW(
	const Recti& bounds, Context context, SWHolderType inHolderType)
	: SubWindow{ESWTypes::LocalFiles, bounds, context, inHolderType}
	, mCurrentDir{FsFile::InitFile(GetContext().fsClient)}
	, mFileSelector{Recti{
...
void LocalFilesSW::InitializeHotKeysActions()
{
...
void LocalFilesSW::InitializeInitialState()
{
...
void LocalFilesSW::HandleUserEvent(Event event, bool rawInputMode)
{
	SubWindow::HandleUserEvent(event, rawInputMode);
}

void LocalFilesSW::HandleSWEvents(std::vector<SWEvent>& swEvents)
{
}

void LocalFilesSW::Update()
{
	mFileSelector.Update();
}

void LocalFilesSW::Draw() const
{
	if (!IsVisible())
		return;

	SubWindow::DrawBoundsRectangle("Local");
	mFileSelector.Draw();
}

void LocalFilesSW::OpenSelectedDirectory()
{
	const File_Ptr file = mFileSelector.GetSelected().value();
...
void LocalFilesSW::HandleResizeEvent(const Recti& newRect)
{
...
// void LocalFilesSW::setVisible(bool visibleValue)
//{
